...
## 2026-07-08 - [Bulk Database Cleanup & Empty List Safe Guards]
**Инсайт:** Извлечение всех записей из базы данных в память для фильтрации и сопоставления неактивных пользователей вызывает значительные задержки и нагрузку на CPU/память. Использование специализированного bulk-запроса `deleteExceptUserIds` полностью устраняет эту нагрузку. Кроме того, Room/SQLite некорректно обрабатывает `NOT IN` с пустым списком, поэтому для безопасного удаления активных пользователей с нулевым списком приложений необходимо применять явную проверку в коде и делать fallback на удаление по ID пользователя (`deleteByUserId`).
**Действие:** Всегда использовать групповые операции в SQLite для очистки и синхронизации данных. При использовании `NOT IN` запросов всегда обрабатывать пограничный случай пустой коллекции в Kotlin, переводя логику на альтернативный запрос, чтобы избежать аномального поведения СУБД.
