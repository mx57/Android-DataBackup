...
## 2026-07-08 - [Robust Database Sync & Empty Collections]
**Инсайт:** SQLite/Room's `NOT IN` queries with an empty collection return zero matches and delete nothing rather than matching everything. Moreover, database synchronization logic can leave orphaned rows for deleted users since their package names are completely omitted from the active package stream.
**Действие:** Designed an robust sync flow where we check current active user IDs from root-level service against the local database to purge inactive users (`deleteByUserId`). For active users, explicitly branch on empty package name lists in Kotlin to invoke a full user cleanup (`deleteByUserId`) instead of evaluating the Room `deleteExcept` query with an empty collection.
