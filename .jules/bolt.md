...
**Инсайт:** Когда внедряется массовый выбор (выделить всё, снять выделение, инвертировать) на экране с фильтрами и поиском, выполнение этих операций на всём списке приложений игнорирует отфильтрованное состояние пользователя. Работа напрямую с отфильтрованным списком имен пакетов из реактивного StateFlow во ViewModel и выполнение атомарных пакетных обновлений в SQLite (`UPDATE ... WHERE packageName IN (:packageNames)`) предотвращает несогласованное состояние интерфейса и накладные расходы O(N) на итерацию в коде.
**Действие:** Реализован оптимизированный `reverseSelection` в `AppDao`, который инвертирует все флаги выбора одновременно. Добавлены методы `selectAllFiltered` и `reverseSelectionFiltered` во `AppsViewModel` для обновления БД по списку пакетов из отфильтрованного потока. Кнопки действий "Выделить всё", "Снять выделение" и "Инвертировать выбор" добавлены в нижнюю панель с использованием неперсистентного стиля (`selected = false`).

## 2026-07-08 - [Robust Multi-User Sync & SQLite NOT IN Limitation]
**Инсайт:** При синхронизации установленных приложений в многопользовательской среде удаление неактивных/удаленных пользователей, а также пользователей без установленных приложений, может приводить к утечке данных в локальной БД или Room-ошибкам. SQLite-запрос `NOT IN (:emptyCollection)` возвращает UNKNOWN и ничего не удаляет, оставляя осиротевшие записи.
**Действие:** Сначала выявлять активных пользователей через рут-сервис, удалять данные неактивных пользователей через `deleteExceptUserIds(activeUserIds)`, а для активных пользователей использовать ветвление: вызывать `deleteExcept(packageNames, userId)` только если список приложений не пуст, иначе сбрасывать данные через `deleteByUserId(userId)`.
